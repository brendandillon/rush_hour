...
class ServerControllerTest < Minitest::Test
  ...
  def test_it_knows_if_payload_data_has_not_been_received
    post '/sources', {"rootUrl" => "abc.com", "identifier" => "abc"}
    get '/sources/abc'
  ...
  def test_something
    post '/sources', {"rootUrl" => "abc.com", "identifier" => "abc"}
    post '/sources/something/data', "payload" =>'{"url":"http://jumpstartlab.com/blog","requestedAt":"2013-02-16 21:38:28 -0700","respondedIn":37,"referredBy":"http://jumpstartlab.com","requestType":"GET","userAgent":"Mozilla/5.0 (Macintosh%3B Intel Mac OS X 10_8_2) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1309.0 Safari/537.17","resolutionWidth":"1920","resolutionHeight":"1280","ip":"63.29.38.211"}'

    assert_equal 403, last_response.status
    assert_equal true, last_response.body.include?("Error 403:")
  end
end
